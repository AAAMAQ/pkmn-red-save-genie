...
static void WritePokemonBoxJsonObject(std::ofstream& f, const savegenie::PokemonBox& box, const std::string& indent) {
    f << indent << "{\n";
...
static bool WriteTerminalTranscriptExport(ScopedTerminalTranscript& transcript, const std::string& outPath) {
    ...
    if (!WriteSaveGenieSummaryTxt(transcript.Text(), outPath)) {
        std::cout << "\n[ERROR] Failed to write terminal summary export: " << outPath << "\n";
        WriteSaveGenieSummaryTxt(transcript.Text(), outPath);
        return false;
    }

    std::cout << "\n[OK] Wrote terminal summary export: " << outPath << "\n";
    return WriteSaveGenieSummaryTxt(transcript.Text(), outPath);
}
